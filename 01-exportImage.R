...
# Database outside of repository
image <- read_sav("../base_final_anon.sav")

region <- read_dta('pride_protests.dta')

image <- image %>% 
  left_join(region %>% select(id, webal, geozone),
            by = 'id')
